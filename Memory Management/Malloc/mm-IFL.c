/*
 * mm-IFL.c - Using implicit free list and implement all three fit strategy.
 * However, the performance of this implementation is not good(detailed in
 * report).
 */
#include <assert.h>
#include <stddef.h>
...
// extend the heap by creating a new block and a new end block
// return the start address of the new block after merge
static void *extend_heap(size_t heap_size);

// merge the block with its previous and next block if they are free
// always input a new free block
...
static void *extend_heap(size_t heap_size) {
  ...
  // we don't move the new_ptr forward 4 bytes because we use the place of the
  // old end block as the new block's header, both 4 bytes
...
/* first fit */
  ...
  while (GET_SIZE(HEADER(ptr)) != 0) {
    if ((GET_ALLOC(HEADER(ptr)) == 0) && GET_SIZE(HEADER(ptr)) >= block_size)
      return ptr;
    else
      ptr = NEXT_BLOCK(ptr);
  }
...
/* next fit */
#ifdef NEXT_FIT
  for (ptr = fit_start; GET_SIZE(HEADER(ptr)) > 0; ptr = NEXT_BLOCK(ptr)) {
    if (GET_ALLOC(HEADER(ptr)) == 0 && GET_SIZE(HEADER(ptr)) >= block_size) {
      fit_start = ptr;
      return ptr;
    }
  ...
  for (ptr = heap_list; ptr != fit_start; ptr = NEXT_BLOCK(ptr)) {
    if (GET_ALLOC(HEADER(ptr)) == 0 && GET_SIZE(HEADER(ptr)) >= block_size) {
      fit_start = ptr;
      return ptr;
    }
...
/* best fit */
  ...
  char *best_ptr = NULL;
  size_t min_size = 0;
  for (ptr = heap_list; GET_SIZE(HEADER(ptr)) != 0; ptr = NEXT_BLOCK(ptr)) {
    if ((GET_ALLOC(HEADER(ptr)) == 0) && GET_SIZE(HEADER(ptr)) >= block_size) {
...
static void set_block(void *ptr, size_t block_size) {
  ...
  // if the block size is larger than the required size, split the block
...
void *malloc(size_t size) {
  ...
  // if there is no fitted block, allocate more memory and place the block
...
/*
 * free - Just return the block and try to merge with pre or next block.
 */
void free(void *ptr) {
  if (ptr == NULL)
    return;
...
void mm_checkheap(int verbose) {
...
#ifdef NEXT_FIT
  // check if fit_start is in the heap
  ptr = heap_list;
  while (GET_SIZE(HEADER(ptr)) != 0) {
    if (ptr == fit_start)
      break;
  ...
  if (GET_SIZE(HEADER(ptr)) == 0)
    printf("fit_start is not in the heap\n");
#endif
}
