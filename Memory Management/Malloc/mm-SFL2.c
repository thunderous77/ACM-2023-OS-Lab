/*
 * mm-SFL2.c - merge mm-EFL5.c and mm-SFL.c. However, the performance on
 * throughput is worse than mm-EFL5.c.
 */
#include <assert.h>
#include <stddef.h>
#include <stdio.h>
...
#define CLASSNUM 10        /* number of classes -- (0,32], (32, 64] ... */
...
static void remove_free_block(void *ptr) {
  ...
  if (prev_free_block == NULL && next_free_block == NULL) {
    SET_CLASS_HEAD(class_num, 0);
  } else if (prev_free_block == NULL) {
    SET_CLASS_HEAD(class_num, (long)next_free_block);
 ...
 */
int mm_init(void) {
  if ((heap_list = mem_sbrk((4 + CLASSNUM) * WSIZE)) == (void *)-1)
    return -1;
  // init heap
  for (int i = 0; i < CLASSNUM; i++) {
    WRITE(heap_list + (i * WSIZE), 0);
  }
  WRITE(heap_list + CLASSNUM * WSIZE, 0);
  WRITE(heap_list + CLASSNUM * WSIZE + (1 * WSIZE), PACK(BSIZE, 1));
  WRITE(heap_list + CLASSNUM * WSIZE + (2 * WSIZE), PACK(BSIZE, 1));
  WRITE(heap_list + CLASSNUM * WSIZE + (3 * WSIZE), PACK(0, 1));
 ...
 * 1. The prologue block is 8 byte and allocated(prevent merge).
...
void mm_checkheap(int verbose) {
  ...
  if (GET_SIZE(HEADER(heap_list_shadow)) != BSIZE ||
      GET_ALLOC(HEADER(heap_list_shadow)) != 1 ||
      GET_SIZE(FOOTER(heap_list_shadow)) != BSIZE ||
